...
entropy_scheduler = {                               # Entropy scheduler parameters TODO: entropy_weight should factor in batch size and expected reward scale
    ...
    'minimum_entropy_weight': 1e-4
}
entropy_weights_per_head = {'structure': 100., 'continuous': 1., 'discrete': 0.0, 'input_influence': 0.0} 
...
ic_values_diagonal_1 = [[x, x-1] for x in range(1, 11)]
ic_values_diagonal_2 = [[x, x+1] for x in range(0, 10)]
ic_values_cluster_1 = [ [9, 0], [10, 0], [11, 0], 
                        [9, 1], [10, 1], [11, 1]]
ic_values_cluster_2 = [ [0, 9], [0, 10], [0, 11], 
                        [1, 9], [1, 10], [1, 11]]
...
r_list = [np.array([10, 0])] * len(ic_values_diagonal_1) + \
         [np.array([0, 10])] * len(ic_values_diagonal_2) + \
         [np.array([10, 0])] * len(ic_values_cluster_1) + \
         [np.array([0, 10])] * len(ic_values_cluster_2)

# Construct the weights for the performance metric
w = np.ones(N_t)
# w[(len(w)//5)*4:] = w[(len(w)//5)*4:]*2
# w[:(len(w)//5)] = w[:(len(w)//5)]*0.25
w[(len(w)//5)*4:] = w[(len(w)//5)*4:]*5
w[:(len(w)//5)*4] = w[:((len(w)//5)*4)]*0
w = w[np.newaxis, :]
