...
logger = CometLogger(
    api_key=api_key,
    project="multistability_3species_8rxns_MAK_REINFORCE",        
    workspace="maurice-filo", 
    name=f'multistability_3species_8rxns_MAK_REINFORCE_{timestamp}',
)
...
r3 = MassAction(reactant_labels=['X_3'], product_labels=[], input_channels=[None], params=[0.1], params_controllability=[True])
...
file_name = "multistability_3species_8rxns_MAK_REINFORCE.xlsx"  # Filename for saving the Excel sheet
...
ic_values_numpy = np.array(ic_values)
fig = plt.figure(figsize=(8, 7))
ax = fig.add_subplot(111, projection="3d")
ax.scatter(ic_values_numpy[:,0], ic_values_numpy[:,1], ic_values_numpy[:,2])
