...
CAMPAIGN_ID=flash-rpa-direct-nonthinking-100epoch-20seed
...
printf '[%s] Launching 20-seed RPA direct non-thinking campaign.\n' "$(date --iso-8601=seconds)"
"$ROOT/.venv/bin/python" "$LAUNCHER" \
    ...
    --method-name genai_net_llm_flash_rpa_direct_nonthinking100 \
    --run-suffix cvode_flash_rpa_direct_nonthinking100 \
    --dsh-home "$DSH_HOME" \
    --workspace-root "$CAMPAIGN_ROOT" \
    --comet-project genai-net-v4-flash-paper \
    --skip-postprocessing

printf '[%s] RPA direct non-thinking campaign completed.\n' "$(date --iso-8601=seconds)"
