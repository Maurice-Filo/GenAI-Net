...
CAMPAIGN_ID=flash-classifier-no-communication-100epoch-20seed
WAIT_PID="${WAIT_PID:-}"
RL_GPU_INDEX="${RL_GPU_INDEX:-0}"

if [[ -n "$WAIT_PID" ]]; then
    printf '[%s] Waiting for active campaign PID %s before classifier ablation.\n' \
...
printf '[%s] Launching 20-seed classifier no-communication ablation on GPU %s.\n' \
...
exec "$ROOT/.venv/bin/python" "$LAUNCHER" \
    ...
    --method-name genai_net_llm_flash_classifier_no_communication100 \
    --run-suffix cvode_llm_flash_classifier_no_communication100 \
    --dsh-home "$DSH_HOME" \
