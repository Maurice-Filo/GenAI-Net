...
CAMPAIGN_ID=flash-rpa-direct-nonthinking-hof-reconnected-100epoch-20seed
...
printf '[%s] Launching 20-seed RPA non-thinking campaign with CRN-valued HOF context.\n' \
...
"$ROOT/.venv/bin/python" "$LAUNCHER" \
    ...
    --generation-backend direct-nonthinking \
    --llm-provider deepseek-official \
    --communication-mode full \
    --withhold-initial-hof \
    --method-name genai_net_llm_flash_rpa_direct_nonthinking_hof100 \
    --run-suffix cvode_flash_rpa_direct_nonthinking_hof100 \
    --dsh-home "$DSH_HOME" \
    --workspace-root "$CAMPAIGN_ROOT" \
    --comet-project genai-net-v4-flash-paper \
    --skip-postprocessing

printf '[%s] RPA non-thinking HOF-context campaign completed.\n' "$(date --iso-8601=seconds)"
