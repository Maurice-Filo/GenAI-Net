...
def _final_loss(task: str, method: str, seed: int) -> float:
    run_id = _method_run_ids(task, seed + 1)[method][seed]
...
def _write_pairwise_stats(figure_dir: Path, n_seeds: int) -> Path:
    ...
    out = figure_dir / f"rpa_logic_genai_julia_{n_seeds}seed_pairwise_stats.csv"
    fields = [
        "task",
        "n_runs",
        "genai_mean",
    ...
    with out.open("w", newline="", encoding="utf-8") as f:
        ...
        for task in ("rpa", "logic"):
            genai = np.array([_final_loss(task, "rl4crn", seed) for seed in range(n_seeds)], dtype=float)
            julia = np.array([_final_loss(task, "reaction_network_evolution_jl", seed) for seed in range(n_seeds)], dtype=float)
            diff = julia - genai
            boot = []
...
def main() -> None:
    ...
    parser.add_argument("--figure-name", default="rpa_logic_genai_julia_20seed_side_by_side.png")
    ...
    out = plot_side_by_side_seed_summary(
        [
            _panel("rpa", "RPA task", args.n_seeds),
            _panel("logic", "Logic circuit task", args.n_seeds),
        ],
        figure_dir,
    ...
    for task in ("rpa", "logic"):
        summary_path = figure_dir / f"{task}_genai_julia_{args.n_seeds}seed_summary.csv"
        write_seed_summary(
