...
default_checkpoint_path = "Quadratic_task_chkpt_run_Quad_8S_16R-11.pkl"
...
cfg.solver.rtol = 1e-7
cfg.solver.atol = 1e-7

# build template IO/CRN
species_labels = ['X_1', 'X_2', 'X_3', 'X_4', 'X_5', 'X_6', 'X_7', 'X_8']
...
def quadratic_residual_reward(state):
    ...
    lambda_quad = task.params.get("residual_weight", 1.0)
    lambda_ss = task.params.get("steady_state_weight", 1.0)
    lambda_size = task.params.get("size_weight", 1e-4)
    hidden_cap = task.params.get("hidden_cap", 20.0)
...
# %%
# ---- Train config ----
cfg.train.max_added_reactions = 18
