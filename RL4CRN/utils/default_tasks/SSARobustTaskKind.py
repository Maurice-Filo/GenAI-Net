...
import numpy as np
from RL4CRN.rewards.stochastic import robust_tracking_loss_SSA

@register_task_kind
class SSARobustTaskKind(TaskKindBase):
    """Robust SSA tracking loss (CV + RPA) with named-expression targets (or constant)."""
    kind = "ssa_robust"
    ...
    def default_u_list(self, task: TaskSpec) -> List[np.ndarray]:
        ...
        if u_values is None:
            raise ValueError("ssa_robust default_u_list requires params['u_values'].")
        if task.n_inputs is None:
            raise ValueError("ssa_robust default_u_list requires task.n_inputs.")
    ...
    def validate(self, task: TaskSpec) -> None:
        ...
        if target is None:
            raise ValueError("ssa_robust task requires params['target'].")
        if not (callable(target) or isinstance(target, (int, float))):
            raise ValueError("ssa_robust target must be a float/int or a callable with named args.")
    ...
    def make_reward_fn(self, task: TaskSpec, overrides: Dict[str, Any]) -> Callable[[Any], Any]:
        ...
        if target is None:
            raise ValueError("ssa_robust task requires target.")
        ...
        relative = overrides_get(task, overrides, "relative", fallback_attr="relative", default=task.relative)
        ...
        cv_weight = float(overrides_get(task, overrides, "cv_weight", fallback_attr="cv_weight", default=task.cv_weight))
        rpa_weight = float(overrides_get(task, overrides, "rpa_weight", fallback_attr="rpa_weight", default=task.rpa_weight))
