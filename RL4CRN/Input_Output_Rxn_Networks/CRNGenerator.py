...
class CRNGenerator(torch.nn.Module):
    def __init__(self, num_species, num_reactions, num_free_parameters, num_inputs, parameter_grid, num_samples, RSG_Attributes, PSG_Attributes, device='cpu', allow_input_influence=False):
        super().__init__()
        self.device = device
        ...
        if self.allow_input_influence:
            num_categories = [(num_species + 1) * (num_species + 2) // 2, (num_species + 1) * (num_species + 2) // 2, num_inputs + 1]
        else:
            num_categories = [(num_species + 1) * (num_species + 2) // 2, (num_species + 1) * (num_species + 2) // 2]
...
class CRNCompletor(CRNGenerator):
    ...
    def __init__(self, max_num_reactions, num_species, num_reactions, num_free_parameters, num_inputs, parameter_grid, num_samples, RSG_Attributes, PSG_Attributes, device='cpu', allow_input_influence=False):
        super().__init__(num_species, num_reactions, num_free_parameters, num_inputs, parameter_grid, num_samples, RSG_Attributes, PSG_Attributes, device, allow_input_influence=allow_input_influence)
        self.RSG_Attributes = RSG_Attributes
        self.PSG_Attributes = PSG_Attributes
        self.max_num_reactions = max_num_reactions
        self.MISG = MultIndexSequenceGenerator(RSG_Attributes.LSTM_hidden_size, RSG_Attributes.FFNN_hidden_size, self.num_categories, RSG_Attributes.FFNN_num_layers, max_num_reactions, num_samples, RSG_Attributes.weight)
        self.ISG = IndexSequenceGenerator(PSG_Attributes.LSTM_hidden_size, PSG_Attributes.FFNN_hidden_size, self.parameter_grid_size, PSG_Attributes.FFNN_num_layers, max_num_reactions+num_free_parameters, num_samples, PSG_Attributes.weight)

    def forward(self):
        h0 = torch.ones(self.num_samples, self.RSG_Attributes.LSTM_hidden_size, requires_grad=False).float().to(self.device)
        c0 = torch.ones(self.num_samples, self.RSG_Attributes.LSTM_hidden_size, requires_grad=False).float().to(self.device)
        indices_structure, total_logP, total_entropy, h, c = self.MISG(h0, c0, logP0=0, H0=0, I0=None, index_start=None)
        ...
        for i in range(self.num_samples):
            ...
            for j in range(self.max_num_reactions):
                action_set.append({ 
                    'reactants index': indices_structure[i, 0, j].cpu().numpy(),
                    'products index': indices_structure[i, 1, j].cpu().numpy(),
                    'input influence index': indices_structure[i, 2, j].cpu().numpy() if self.allow_input_influence else 0,
