...
class ParameterGeneratorFromDistribution(torch.nn.Module):
    def __init__(self, distribution, backbone, device='cpu'):
        ...
        # Initialize the attributes and move to device
        ...
        if distribution["type"] == 'lognormal':
            ...
            self.decoder = FFNN(input_size=backbone.output_size, output_size=D + D + (D**2 - D)//2, hidden_size=backbone.hidden_size, num_layers=1).to(device=device)  
            ...
            # self.forward = lognormal_forward.__get__(self)
                ...
                # Run the decoder to get the unprocessed distribution parameters
                params = self.decoder(self.backbone(x)) # shape: (N, D + D + (D**2 - D)//2)
